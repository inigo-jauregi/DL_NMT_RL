...
if opt.train_part not in ["all", "context", "sentences"]:
	sys.stderr.write("Invalid option for train_part\n")
	sys.exit(1)

elif opt.train_part == "context" and (opt.train_from == '' or not opt.context_type):
	sys.stderr.write("Cannot train context alone or in this context_type model\n")
	sys.exit(1)
...
def report_func(epoch, batch, num_batches,
	...
	if batch % opt.report_every == -1 % opt.report_every:
		if opt.train_type == 'REINFORCE':
			report_stats.output_REINFORCE(epoch, batch + 1, num_batches, start_time)
		else:
...
def make_loss_compute(model, tgt_vocab, opt, train=True, RL_loss = False):
	"""
	...
	if opt.copy_attn:
		...
		if RL_loss == False:
			...
			compute = onmt.Loss.REINFORCELossCompute(model.generator, tgt_vocab)
...
def train_model(model, fields, optim, data_type, model_opt, train_part,batch_size=None):
...
def build_model(model_opt, opt, fields, checkpoint):
	...
	model = onmt.ModelConstructor.make_base_model(model_opt, fields,
												  use_gpu(opt), checkpoint, opt.train_part)
	if len(opt.gpuid) > 1:
...
def main():
	...
	#print (first_dataset.doc_index) #Dataset has document index
