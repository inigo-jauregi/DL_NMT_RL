...
class TransformerDecoderLayer(nn.Module):
	...
	def forward(self, inputs, memory_bank, src_pad_mask, tgt_pad_mask,
		...
		return output, attn, all_input, mid
...
class TransformerDecoder(nn.Module):
	...
	def __init__(self, num_layers, hidden_size, attn_type,
		...
		if copy_attn:
			...
			self._copy = True
		self.layer_norm = onmt.modules.LayerNorm(hidden_size)
		self.layer_norm_c = onmt.modules.LayerNorm(hidden_size)
	...
	def forward(self, tgt, memory_bank, state, memory_lengths=None):
		...
		mid = self.layer_norm_c(mid)

		# Process the result and update the attentions.
		outputs = output.transpose(0, 1).contiguous()
		attn = attn.transpose(0, 1).contiguous()
		mids = mid.transpose(0, 1).contiguous()
		...
		return outputs, state, attns, mids
