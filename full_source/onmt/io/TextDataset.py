	...
	""" Dataset for data_type=='text'
		...
		Args:
			...
			use_filter_pred (bool): use a custom filter predicate to filter
				out examples?
	"""
	def __init__(self, fields, src_examples_iter, tgt_examples_iter, doc_index,
		...
		#Each example has -> {'src':(token1, token2,...), 'indices': 0 , 'tgt':(token1,token2,...)}
		#The index is per sentence from 0 to 209,000
		keys = ex.keys()

		out_fields = [(k, fields[k]) if k in fields else (k, None)   #Just collect the fields and their types
					  for k in keys]
		example_values = ([ex[k] for k in keys] for ex in examples_iter)
		#Remove dictionary: [(token1, token2,...),0 ,(token1,token2,...)]
		...
		out_examples = []  #Create a list of objects which class is Example (ex.src, ex.indices and ex.tgt)
		...
		filter_pred = filter_pred if use_filter_pred else lambda x: True #Give a function if we want to filter

		doc_index = np.array(doc_index)
		if use_filter_pred:
			for ex in out_examples:
				if not filter_pred(ex) and ex.indices in doc_index:   #Check if we need to filter
					doc_index[doc_index==ex.indices]+=1


		self.doc_index = sorted(list(set(doc_index)))  #Store the doc_index
